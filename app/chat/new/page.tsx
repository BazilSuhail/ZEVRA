              ...
              {selectedUsers.map((id) => {
                ...
                return user ? (
                  <motion.span
                    ...
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400"
                  >
                    {user.name}
          ...
          {filtered.map((user, i) => (
            <motion.div
              ...
              className={`flex items-center justify-between rounded-xl px-4 py-3 transition-colors ${
                selectedUsers.includes(user.id) ? "bg-emerald-50 dark:bg-emerald-900/20" : "hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
              }`}
            >
              <div className="flex items-center gap-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => (isGroup ? toggleUser(user.id) : null)}
