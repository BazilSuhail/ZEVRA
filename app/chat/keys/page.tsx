                ...
                {keys.map((k, i) => (
                  <motion.div key={k.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="rounded-xl bg-zinc-50 p-4 dark:bg-zinc-800">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      {k.verified ? (
                        <span className="flex items-center gap-1 text-xs text-emerald-500"><FiCheckCircle className="h-3 w-3" />Verified</span>
                      ) : (
                    ...
                    {!k.verified && (
                      <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="mt-3 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700">Verify</motion.button>
                    )}
                  </motion.div>
                ))}
              </div>
